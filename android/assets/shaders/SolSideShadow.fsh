...
const float reach = 1600;//How far down the shadow reaches.
...
void main(void) {
	...
	vec2 coord = vec2(v_texCoords.x*(u_size.x/u_screensize.x)-xdis, 1-ysamp);
    vec4 texColor1 = texture2D(u_texture1, coord);

	float mixAlpha = texColor1.a*0.9*alpha*(1.0-v_texCoords.y/(reach*(1-texColor1.rgb)/u_size.y));
	...
	gl_FragColor.g = gl_FragColor.g*0.5*(1.0-v_texCoords.y*fadeFactor)+gl_FragColor.g*0.5;
	gl_FragColor.b = gl_FragColor.b*0.2*(1.0-v_texCoords.y*fadeFactor)+gl_FragColor.b*0.8;
	gl_FragColor.r = gl_FragColor.r*0.15*(1.0-v_texCoords.y*fadeFactor)+gl_FragColor.r*0.85;
