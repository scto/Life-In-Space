...
public class ButtonSave extends Entity {
	...
	public ButtonSave(String n, float x, float y) {
	...
	public void render(Level lvl) {
		...
		LM.brightShader.setUniformf("alpha", alpha*(scale < 1f ? 0.4f : 1f));
	...
	public void press(Level lvl, int id){
		Main.clickSound.play(0.5f, 3f, 0.5f);
		pressed = true;
		scale = 0.95f;
		if(id == 0){
	...
	public void init(Level lvl) {
		LM.loadTexture("img/buttonSave.png");
	}

	public boolean doneLoad(Level lvl) {
		if(LM.loader.isLoaded("img/buttonSave.png")){
			norm = new TextureRegion(LM.loader.get("img/buttonSave.png", Texture.class));
			draw = norm;
